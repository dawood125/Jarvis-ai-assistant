...
function IntelPanel({
  ...
  const [activeTab, setActiveTab] = useState(focusSection || 'actions')
  const showDevPanel = import.meta.env.VITE_SHOW_DEV_PANELS !== 'false'
  const safeNotes = Array.isArray(notes) ? notes.slice(0, 4) : []

  useEffect(() => {
    if (focusSection && focusSection !== activeTab) {
  ...
  return (
    ...
    >
      ...
      {activeTab === 'memory' && (
        <motion.section variants={itemVariants} className="glass-panel p-4">
          <h2 className="panel-title">User Memory</h2>
          ...
          <div className="mt-4">
            ...
            <div className="space-y-2">
              {safeNotes.length === 0 && (
                <div className="rounded border border-cyan-900 bg-slate-900/30 p-2 text-[10px] text-slate-500">
              ...
              {safeNotes.map((note) => (
                <div
                  ...
                  className="rounded border-l-2 border-cyan-500 bg-slate-900/30 p-2 text-[11px] text-slate-300"
                >
                  {note.text}
                </div>
              ))}
